...
export async function getTopCommissariatsLeaderboard() {
  // Aggregate articles by commissariat
  ...
  // Fetch commissariat names since groupBy doesn't include relations
...
export async function getCommissariatDashboardStats(commissariatId: string) {
  const [
    publishedArticles,
    draftArticles,
    rejectedArticles,
  ...
  // Get active verifications/profiles count to know if they have pending submissions
