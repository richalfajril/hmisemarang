...
- **Generate Carousel (Artikel → Carousel Instagram) (2026-07-19):** Fitur baru `features/carousel-generator` mengubah artikel PUBLISHED menjadi carousel IG (1080×1350, ZIP) dari menu Aksi tabel artikel (ADMIN_CABANG/SYSTEM_ADMIN). **Deviasi dari draft spec** (`FEATURE_GENERATE_CAROUSEL.md`): rendering **client-side** (`html-to-image`) menggantikan Playwright/Chromium server-side yang tak cocok Vercel serverless; **block-level pagination** (tiap blok Tiptap diukur & disusun ke slide tanpa dipecah, `lib/paginate.ts` + self-check, cap 20 slide) menggantikan pemotongan offset-tetap; QR via `qrcode` → `/artikel/{slug}`, ZIP via `jszip`, unduh langsung tanpa temporary asset Cloudinary/endpoint `/api`. Header IG **global** di `website_settings.carousel_header_url` (kolom baru, `prisma db push`) via `saveCarouselHeaderAction`. Dep baru client-only disetujui: `html-to-image`, `qrcode`, `jszip`.
- **Review Center: Fix Redirect 404 Setelah Kirim Keputusan (2026-07-19):** Halaman `review-center/[type]/[id]` kini redirect ke `/dashboard/review-center` saat entitas sudah tak reviewable (implicit refresh Server Action) alih-alih memicu `notFound()`/404; `notFound()` hanya untuk ID yang tak ada. Berlaku 4 tipe entitas.
